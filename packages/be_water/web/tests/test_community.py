...
def _water(wid, added_by, province, verified_fields=(), photo=None, added_at=None):
    return Water(
        id=wid,
        name=wid,
        brand=wid,
...
def test_scores_and_ranking_order():
    ...
    assert ranking[0]["score"] == 4  # 2 waters + 2 verified fields
    assert ranking[1]["score"] == 1
...
def test_achievements_fire_on_thresholds():
    catalog = [
        _water(
            ...
            "jorgelillo",
            province,
            verified_fields=["tds", "sodium"],
            photo="https://x/p.jpg",
        )
        for i, province in enumerate(["Granada", "Toledo", "Cuenca"])
    ]
    ranking = build_community_stats(catalog, "2026-07")
    names = {b["name"] for b in ranking[0]["badges"]}
    assert {"Primera gota", "Ojo de halcón", "Cartógrafo", "Paparazzi"} <= names
