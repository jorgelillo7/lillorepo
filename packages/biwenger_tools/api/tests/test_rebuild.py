...
def test_store_writes_a_thin_season_scoped_document(monkeypatch):
    ...
    fake = _FakeFirestore()
    monkeypatch.setattr(rebuild, "fs", fake)

    plan_id = rebuild.store(_stored_plan())

    path = rebuild._plans_path(config.CURRENT_SEASON)
...
def test_load_returns_the_stored_plan(monkeypatch):
    fake = _FakeFirestore()
    monkeypatch.setattr(rebuild, "fs", fake)

    plan_id = rebuild.store(_stored_plan())

    assert rebuild.load(plan_id)["formation"] == "3-4-3"
    assert rebuild.load("does-not-exist") is None


def test_discard_deletes_so_the_plan_cannot_run_twice(monkeypatch):
    fake = _FakeFirestore()
    monkeypatch.setattr(rebuild, "fs", fake)

    plan_id = rebuild.store(_stored_plan())
    assert rebuild.load(plan_id) is not None

    rebuild.discard(plan_id)

    assert rebuild.load(plan_id) is None
