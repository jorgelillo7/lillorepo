...
| 🔨 | International waters — the `country` field exists, nothing uses it | Nothing: `FONTÉBIL` is Portuguese and its ficha says `province='portugal', community='portugal', country='ES'` · `country` decides whether a community is expected at all · [analysis](docs/technical/parked-work.md#be_water-country-field) |
...
| 🔨 | The composition shot and the origin panel are different faces of the bottle | Nothing · **measured:** the same prompt reads `Badajoz`/`Extremadura` off the back panel and `None` off the photo actually stored · it is not the OCR · [evidence](docs/technical/parked-work.md#be_water--the-origin-the-camera-never-saw) |
| 🔨 | A second, optional label photo for the origin panel | Nothing: **the trigger fired** · `fuente-dehesa` hit `verified=True` with no origin because the reader was never shown that face · [why parked](docs/technical/parked-work.md#be_water--a-second-optional-label-photo) |
