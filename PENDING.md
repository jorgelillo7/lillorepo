...
| 🔨 | A second, optional label photo for the origin panel | Nothing: **the trigger fired** · `fuente-dehesa` hit `verified=True` with no origin — measured, the same prompt reads `Badajoz` off the other face and `None` off the photo stored · [why parked](docs/technical/parked-work.md#be_water--a-second-optional-label-photo) |
