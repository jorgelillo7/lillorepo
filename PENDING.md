...
| 🔨 | International waters — the `country` field exists, nothing uses it | Nothing: `FONTÉBIL` is Portuguese and its ficha says `province='portugal', community='portugal', country='ES'` · the EU PDF already carries every member state and the parser throws them away · [analysis](docs/technical/parked-work.md#be_water-country-field) |
| 🔨 | The OCR reads no origin off a bottler address | Nothing · `Una Dehesa` saved with empty spring **and** province **and** community while `verified=True` · it is the prompt, not a lookup · [why](docs/technical/parked-work.md#be_water--the-province-the-ocr-did-not-read) |
| 🔨 | `data_audit` flags bad minerals but never bad geography | Nothing · both broken fichas pass every check it has · a geo check also gives the admin page its worklist · [why](docs/technical/parked-work.md#be_water--the-curation-engine-cannot-see-a-broken-province) |
