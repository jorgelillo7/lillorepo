...
### Requirement: The refresh is run by hand, and the diff is the news

The snapshot SHALL be regenerated by running
`packages/be_water/scripts/refresh_aesan_snapshot.py` from a workstation, every
few months (the procedure is in `packages/be_water/OPERATIONS.md`). The
generator SHALL refuse to write when the download is not a PDF or the parse
looks wrong (covered above), and a changed snapshot SHALL reach `master` only
through a reviewed pull request: the diff is the news, and a human reads it.

It is manual because the scheduled job that did this could not work. The
Commission's host refuses GitHub-hosted runners — every scheduled run answered
HTTP 429 or an HTML "Sorry" page, while the same download from a workstation
returns the PDF — and the repository does not let Actions open pull requests.
A job that can only ever go red teaches its owner to ignore it, which is the
failure it existed to prevent: a registry that quietly stops changing looks
identical to one nobody is refreshing, and the snapshot once sat eight years
stale on a URL that had begun answering 404.

No scenario: running it is a human step, and claiming a test for it would be
